...
describe('auth', () => {
    ...
    it('logout', async () => {
        ...
        const profile = await jutge.student.profile.get(null)
        expect(profile.user_uid).toBe(USER_UID)
        await jutge.logout()
        expect(jutge.student.profile.get(null)).rejects.toThrow(UnauthorizedError)
    })
})
...
describe('student', () => {
    ...
    describe('profile', () => {
        ...
        it('get avatar', async () => {
            const [_, avatar] = await jutge.student.profile.getAvatar(null)
        ...
        it('update and restore', async () => {
            ...
            const original = await updatable.student.profile.get(null)
            ...
            const updated = await updatable.student.profile.get(null)
            ...
            const restored = await updatable.student.profile.get(null)
    ...
    describe('keys', () => {
        it('get', async () => {
            const keys = await jutge.student.keys.get(null)
        ...
        it('get problems', async () => {
            const problems = await jutge.student.keys.getProblems(null)
            expect(problems).toBeArray()
            expect(problems.length).toBeGreaterThan(0)
        })

        it('get enrolled courses', async () => {
            const courses = await jutge.student.keys.getEnrolledCourses(null)
            expect(courses).toBeArray()
            expect(courses).toContain('C00001')
        })

        it('get available courses', async () => {
            const courses = await jutge.student.keys.getAvailableCourses(null)
            expect(courses).toBeArray()
            expect(courses).toContain('C00002')
        })

        it('get lists', async () => {
            const lists = await jutge.student.keys.getLists(null)
    ...
    describe('statuses', () => {
        it('get all', async () => {
            const statuses = await jutge.student.statuses.getAll(null)
    ...
    describe('submissions', () => {
        it('get all', async () => {
            const subs = await jutge.student.submissions.getAll(null)
    ...
    describe('courses', () => {
        it('index enrolled', async () => {
            const courses = await jutge.student.courses.indexEnrolled(null)
            expect(courses).toBeObject()
            expect(courses).toHaveProperty('instructor1:FAKE_COURSE1')
        })

        it('index available', async () => {
            const courses = await jutge.student.courses.indexAvailable(null)
            expect(courses).toBeObject()
        ...
        it('enroll and unenroll', async () => {
            ...
            const enrolled = await jutge.student.courses.indexEnrolled(null)
            expect(enrolled).toHaveProperty('instructor1:FAKE_COURSE2')
            // Restore state
            await jutge.student.courses.unenroll('instructor1:FAKE_COURSE2')
            const after = await jutge.student.courses.indexEnrolled(null)
    ...
    describe('dashboard', () => {
        it('get dashboard', async () => {
            const dashboard = await jutge.student.dashboard.getDashboard(null)
        ...
        it('get stats', async () => {
            const stats = await jutge.student.dashboard.getStats(null)
            expect(stats).toBeObject()
        })

        it('get level', async () => {
            const level = await jutge.student.dashboard.getLevel(null)
            expect(level).toBeString()
        })

        it('get absolute ranking', async () => {
            const ranking = await jutge.student.dashboard.getAbsoluteRanking(null)
            expect(ranking).toBeInteger()
            expect(ranking).toBeGreaterThan(0)
        })

        it('get heatmap calendar', async () => {
            const heatmap = await jutge.student.dashboard.getHeatmapCalendar(null)
            expect(heatmap).toBeArray()
        })

        it('get all distributions', async () => {
            const dists = await jutge.student.dashboard.getAllDistributions(null)
    ...
    describe('awards', () => {
        it('get all', async () => {
            const awards = await jutge.student.awards.getAll(null)
    ...
    describe('lists', () => {
        it('get all', async () => {
            const lists = await jutge.student.lists.getAll(null)
        ...
        it('get', async () => {
            const allLists = await jutge.student.lists.getAll(null)
